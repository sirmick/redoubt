...
| `class` | `system` or `user` | inherited from the parent |
| `first` | flag | runs before every budget without it (R12); set only at creation (`budget_create`) |
...
gets its reserved share); `root` and `system` are class `system` and `first`, `users` is class
`user`; all three have account 0 and no labels. `init` receives handles to all three.
...
and open calls are freed, but the object outlives it until its exit notice is received or dropped;
its PID is freed with the object. Destroying the creator's budget frees the object (R10), killing
the process first if it still runs; then there is no notice. So a notice never allocates.
...
receiver can read: using one of the wrong kind gets `WrongObject`.
...
table. The process object is charged to its creator, not to the budget it runs in, because its
...
It needs no label check: it follows a call that R1 already allowed.
...
`max_transfer` at least its size. Otherwise its sender gets `Refused`, and the kernel moves on to
...
its `receive` still delivers sends, interrupts and notices. `reply` to a `send`'s message id gets
`InvalidArgument`: a send is never an open call.
...
**R12. Scheduling.** Runnable budgets marked `first` run before all others (`init`, the steward and
the drivers run in them: RESOURCES.md). Among the `first` budgets, and among all the others of
either class, one flat stride queue each: run the lowest pass; at every deschedule, pass +=
runtime x `STRIDE` / weight (never 0: a weight-0 budget holds no process); on wake, pass = max(own
pass, current minimum). Within a budget, threads run round-robin. The timer is always armed (slice
end or the next deadline).
...
| `handle_close` | h | - |
| `budget_create` | h(parent), pages, processes, weight, first, labels, account, deadline -> h | R6-R8; class inherited; `first` and labels below; depth < `MAX_DEPTH` |
...
**`budget_create` labels and `first`.** The child's class is its parent's. `first` needs the
caller's own budget to be `first` and the parent to be class `system`. Labels must be a superset of
the parent's (`LabelDenied` otherwise); adding labels needs the caller's own budget to be class
`system`. Either class check failing is `ClassDenied`. Labels are sorted and deduplicated. The
deadline is absolute; `FOREVER` means none.
...
- **Records** (what does not fit in registers: message bodies, a budget's fields, the
  ...
  in the caller's memory, the same on both widths.
...
| `budget_create` | h: `BadHandle`; spec: record, labels over `MAX_LABELS` before deduplication `TooLarge`, the `first` flag or a 32-bit field `InvalidArgument` | `BadHandle`, `WrongObject`, `TooLarge` (the child would be at depth `MAX_DEPTH`), `ClassDenied` (`first`), `LabelDenied` (labels not ⊇ the parent's), `ClassDenied` (labels added), `OutOfMemory` (pages, plus the budget's own page, over the parent's free pages), `OutOfProcesses`, `InvalidArgument` (weight over the parent's free weight) |
...
8. `class(child) = class(parent)`; only a `first` caller creates a `first` budget, and only in a
   system-class parent; `account(child) = account(parent)` unless the parent's is 0.
