...
| 2 Verified boot | (enforced) The loader verifies an Ed25519 signature over `"redoubt.bundle.v1\0" || u64_le(len) || tar`, with `len` measured from the initrd it was handed, never over the bare archive; the preimage is built in one crate (`redoubt/signing`) shared with the bench's signer. One public development key. Open: M-of-N, rollback protection, verifying the loader itself. |
