...
nameable by address; a QEMU test bench with attack tests. In the sibling beamlet repository: a BEAM
...
| [CAPABILITIES.md](CAPABILITIES.md) | Handles, IPC, minting, budget-stamped revocation, principals, agents, projects, the powerbox and approvals. |
| [CONTAINMENT.md](CONTAINMENT.md) | Labels, sessions and vaults, shared servers (label checks, admission, crashes), covert and timing channels, the executable model. |
| [RESOURCES.md](RESOURCES.md) | Budgets, scheduling, the timer. |
| [INIT.md](INIT.md) | After the kernel: init, the steward, keyd, sshd, restarts, the startup block; the worked example. |
| [NAMESPACES.md](NAMESPACES.md) | 9P, per-process namespaces, `/net`, filesystem servers, littlefs. |
| [PACKAGES.md](PACKAGES.md) | Package format, launching, per-principal packages, signer trust, system updates. |
...
| `steward` | Principals, authentication, sessions, the powerbox, launching, packages, audit file | system | designed |
| `sshd` | SSH front door (`sunset`) and the approval sessions | system | designed |
| `gatewayd`, `webd`, `linkd`, `routerd` | LLM gateway; browser GUI; VLAN/filter/QoS; router | system | Later |
...
- **Handle**: an index into a process's kernel-held capability table; unforgeable, meaningless in any
  ...
  server attaches when minting, telling it which grant a request came through. **Stamp**: the budget
  a handle is revoked with.
...
- **Budget**: a kernel container every process lives in; it pays for and bounds everything, carries
  labels and a deadline (RESOURCES.md). **Lease**: a budget with a deadline. **Pass / stride**: the
  per-budget counters of stride scheduling (RESOURCES.md).
- **Principal**: an accountable identity (a human, an agent, or a project). **Sponsor**: the
  ...
  capabilities. **Vault session**: a session carrying the principal's labels (CONTAINMENT.md).
...
- **Powerbox**: the steward's escalation service; grants authority after an out-of-band approval.
  **Approval session**: `ssh approve@box` (routine) or `ssh approve-hs@box` (high-stakes), where only
  the steward talks. **Approver credential**: a FIDO security key used as an SSH `sk-` key with user
  verification (PIN and touch); required for high-stakes approvals.
...
- **Manifest**: (1) the **boot manifest**, the signed description in the boot bundle of the server
  graph, device authority and budgets; (2) a **package manifest**, the list of contents and requested
  capabilities in a package. **Startup block**: the page a parent writes for a new process: its
  namespace table, service handles, arguments (INIT.md).
