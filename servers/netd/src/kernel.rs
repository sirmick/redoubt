...
//! - volatile reads and writes of a register (32-bit and, for the MAC, 8-bit);
...
impl Transport for Device {
    fn reg_read(&self, off: usize) -> Result<u32, Fault> {
        let at = self.regs.at(off, 4).ok_or(Fault::Bounds)?;
        // SAFETY: `at` is inside the mapping `map_device` returned, which stays mapped for the
        // life of this process (nothing here unmaps it), and `Regs::at` checked the whole 32-bit
        // access lies inside it and is 4-byte aligned. Volatile: the value is a device register.
        Ok(unsafe { (at as *const u32).read_volatile() })
    }

    fn reg_read_u8(&self, off: usize) -> Result<u8, Fault> {
        let at = self.regs.at(off, 1).ok_or(Fault::Bounds)?;
        // SAFETY: as in `reg_read`, for one byte, which needs no alignment. The configuration
        // space asks for byte accesses to byte fields (virtio 1.2, §4.2.2.2).
        Ok(unsafe { (at as *const u8).read_volatile() })
    }

    fn reg_write(&self, off: usize, value: u32) -> Result<(), Fault> {
        let at = self.regs.at(off, 4).ok_or(Fault::Bounds)?;
        // SAFETY: as in `reg_read`; the mapping is writable (it is a device's registers).
