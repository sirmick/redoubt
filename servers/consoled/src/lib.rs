//! `consoled`: the ns16550 UART driver, serving `/dev/cons` over 9P (NAMESPACES.md, The
//! console; IO-ARCHITECTURE.md).
//!
...
//!
//! **Until WP-R3, `log-server` keeps the UART.** Nothing starts `consoled` yet: it takes its
//! device handles from a startup block, and no `init` writes one (WP-K4, WP-R3). So the bench's
//! console is still `log-server`, which holds the same ns16550 and the same interrupt through the
//! kernel's interim gift to the bundle's first program, and this crate is only built and host
//! tested. **The two must never run together**: two holders of one device handle both reach the
//! registers (KERNEL-SPEC.md, question 144), and two readers of one receive FIFO would each take
//! half the line. What separates them is that only one of them is ever started — today
//! `log-server`, and from WP-R3 `consoled`, which the manifest gives the UART's handles and whose
//! arrival is what retires `log-server`'s console duties.
//!
...
//! per channel, NAMESPACES.md), not a second reader here.
