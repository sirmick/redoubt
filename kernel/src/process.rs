...
//!
//! # What a process costs (QUESTIONS.md 127)
...
/// The budget the process runs in, while it runs; 0 once it is gone.
const W_BUDGET: usize = 4; // frame + 1
const W_BUDGET_ID: usize = 5;
const W_PID: usize = 6;
/// The exit endpoint, as `process_create` named it; 0 once it has been destroyed.
const W_ENDPOINT: usize = 7; // frame + 1
const W_ENDPOINT_ID: usize = 8;
const W_FLAGS: usize = 9;
const W_CAUSE: usize = 10;
const W_CODE: usize = 11;
const W_BLAMED_ACCOUNT: usize = 12;
const W_BLAMED_NLABELS: usize = 13;
const W_BLAMED_LABELS: usize = 14; // MAX_LABELS words
const WORDS: usize = W_BLAMED_LABELS + MAX_LABELS;
const _: () = assert!(WORDS * 8 <= xous_kernel::arch::PAGE_SIZE);
...
pub struct Proc {
    ...
    /// The budget it runs in, while it runs.
...
impl MemoryManager {
    pub fn process(&self, frame: u32) -> Proc {
        ...
        Proc {
            ...
            budget: frame_of(w(W_BUDGET)).map(|frame| BudgetRef { frame, id: w(W_BUDGET_ID) }),
    ...
    pub fn store_process(&mut self, frame: u32, p: &Proc) {
        ...
        words[W_BUDGET] = p.budget.map_or(0, |b| frame_word(b.frame));
        words[W_BUDGET_ID] = p.budget.map_or(0, |b| b.id);
...
/// until `process_start`. Every frame it takes is charged to the budget it runs in (question 127).
...
fn drop_unstarted(ss: &mut SystemServices, mm: &mut MemoryManager, child: PID) {
    ...
    mm.release_unmapped_frames(child);
...
pub fn process_map(
    ...
    let page_size = xous_kernel::arch::PAGE_SIZE;
    ...
    for i in 0..pages {
        mm.owned_mapping(pid, src + i * page_size)?;
    }
    ...
    let parent = ss.mapping_of(pid).expect("the calling process is alive");
    for i in 0..pages {
        let phys = crate::arch::mem::unmap_from(&parent, src + i * page_size)
...
fn whole_pages(src: usize, dst: usize, len: usize) -> Result<usize, Error> {
    let page = xous_kernel::arch::PAGE_SIZE;
    ...
    if src_end > xous_kernel::arch::USER_AREA_END || dst_end > xous_kernel::arch::USER_AREA_END {
        return Err(Error::InvalidArgument);
    }
    Ok(len / page)
}
...
pub fn process_start(
    ...
    let slots = crate::redoubt::read_slots::<MAX_START_HANDLES>(handles_rec, count)?;
    let mut handles = [None; MAX_START_HANDLES];
    for (i, slot) in slots.iter().enumerate().take(count) {
        handles[i] = Some(mm.handle(pid, AbiHandle::from_raw(*slot)?.index())?);
    }
    if p.started() || !p.alive() {
        return Err(Error::NotPermitted);
    }
...
///
/// The last thread of a process ending is the process ending. Nothing could ever run in it
/// again, so it exits with code 0 rather than lingering with its budget spent and nobody told;
/// the spec does not say, and a process the scheduler can never pick and nobody is owed a notice
/// for would be a leak (reported).
pub fn thread_exit(ss: &mut SystemServices, pid: PID, tid: TID) {
    ...
    if last {
        died(ss, pid, tid, Cause::Exited, 0);
        return;
    }
...
fn record(
    ...
    let flow = mm.budget_of(pid).map(|f| {
        let b = mm.budget(f);
        Flow { class: b.class, labels: b.labels, nlabels: b.nlabels }
    });
    Some((object, flow))
}
...
fn allowed(mm: &MemoryManager, e: EndpointRef, flow: Option<Flow>) -> bool {
    ...
    // R1 checks only between two user budgets; when either side is class `system` it does not
    // (a system server checks itself). `init`, which is system class, reads every notice.
    let Some(f) = flow else { return owner.class == Class::System };
    if owner.class == Class::System || f.class == Class::System {
        return true;
    }
    // Between two user budgets the label sets must be equal: a notice is acted on by whoever
    // reads it, so it is a flow both ways, as a message is.
    owner.labels_of() == &f.labels[..f.nlabels]
}
