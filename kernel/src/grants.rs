...
//! a claim, the way `process_name` reads `PNam`. See `planning/xous64/DEVICE-GRANTS.md`.
...
fn grant_for(pid: PID) -> Option<KernelArgument> {
...
pub fn may_map_device(pid: PID, base: usize, len: usize) -> bool {
    if pid.get() == KERNEL_PID {
        return true;
    }
    let Some(end) = base.checked_add(len) else { return false };
    let Some(grant) = grant_for(pid) else { return false };
...
pub fn may_claim_irq(pid: PID, irq: usize) -> bool {
    if pid.get() == KERNEL_PID {
        return true;
    }
    let Some(grant) = grant_for(pid) else { return false };
