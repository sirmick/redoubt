...
//! and describes each one in the argument block's `Devs` tag (kernel/boot.md); the kernel turns
//! each entry into an object at boot. Nothing else ever creates one, so the set is fixed by the
//! machine and a process reaches a device only through a handle it was given
//! (kernel/devices.md R18).
//!
...
//! charges. The cost table (kernel/objects.md) charges a device like every other object, **one
//! page to the budget that owns it**, which at boot is the budget the loader's programs run in.
//!
...
//! defeat both, and that is the loader's parse to get right (kernel/devices.md, "Residual
//! risks").
//!
//! # A DMA handle is trust
//! An MMIO object with the DMA flag lets its holder call `dma_alloc` and program a bus master with
//! a physical address. On a platform with no IOMMU the device can then write anywhere the kernel
//! gave it. Its frames go back to the pool only after the device confirmed a reset (I16), but
//! nothing stops a live device pointed at the wrong place. That is kernel/devices.md's stated
//! position -- a DMA driver is inside the TCB unless the hardware confines it -- and it means
//! **granting a DMA handle is granting kernel-level trust**. The boot manifest's rules
//! (servers/init.md) are where that belongs to be written down, so a manifest author cannot hand
...
impl MemoryManager {
    ...
    /// `WrongObject` (kernel/abi.md, "Errors and the order of checks").
...
/// controllers (kernel/boot.md, `Ctrl`). The kernel maps the PLIC for itself and the firmware
/// drives the CLINT; a device object naming either would hand its holder every interrupt.
...
impl MemoryManager {
    ...
    ///
    /// **A mapping outlives its handle** (kernel/devices.md, `map_device`). Destroying the owner
    /// budget revokes every handle to the device (R10), so nobody can map it again or receive its
    /// interrupt, but a range already mapped stays mapped until its holder unmaps it or dies. The
    /// kernel keeps no registry of who mapped what, and building one to claw back page entries
    /// would be a mechanism the design does not otherwise have: a device mapping behaves like pages
    /// transferred to another process, which revocation does not reach either. Whoever hands out a
    /// device handle is handing out the device.
    ///
    /// A DMA device joins the set of devices the caller's death must reset before its DMA
    /// frames are pooled: it could be programmed with any of their addresses.
    ...
    /// quarantine destroyed the object (kernel/devices.md, "Quarantine").
    ...
    /// `dma_alloc(h(MMIO), npages) -> addr, phys` (kernel/devices.md, `dma_alloc`):
    ...
    /// hold their address has confirmed a reset (`dma.rs`).
