...
impl MemoryManager {
    ...
    /// budget the process runs in like any other frame it owns (QUESTIONS.md 127: the kernel
    ...
    /// Give back every frame `pid` owns without touching its page tables: a process whose
    /// address space was never finished, because `process_create` ran out of pages building it.
    /// There is no space to walk and nothing has ever been lent, so the second half of
    /// `release_all_memory_for_process` is all that is needed and none of it is unsafe.
    #[cfg(baremetal)]
    pub fn release_unmapped_frames(&mut self, pid: PID) {
        for idx in 0..self.allocations.len() {
            if self.allocations[idx] == Some(pid) {
                self.allocations[idx] = None;
            }
