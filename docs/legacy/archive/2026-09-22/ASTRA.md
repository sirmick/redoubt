> Historical snapshot, 2026-09-22. Superseded descriptions and recommendations are not current contracts.
> Read [STATUS](../../STATUS.md), [open questions](../../QUESTIONS.md), and the owning specifications for current behavior.
...
**Contract.** [KERNEL-SPEC.md](../../KERNEL-SPEC.md), lines 182–189 (R3), says a taken call abandoned by timeout, revocation, or caller death leaves its lend with the server. The caller's ownership/charge ends.

**Implementation.** [kernel/src/message.rs](https://github.com/sirmick/redoubt/blob/main/kernel/src/message.rs), lines 1272–1297, implements abandonment by removing the caller's mapping and moving frames to the server. [kernel/src/arch/riscv/mem.rs](https://github.com/sirmick/redoubt/blob/main/kernel/src/arch/riscv/mem.rs), lines 586–596, implements `drop_lent` with `Pte::EMPTY`. However, [libs/rt/src/ipc.rs](https://github.com/sirmick/redoubt/blob/main/libs/rt/src/ipc.rs), lines 118–132, accepts `Option<&mut Buffer>` and returns on syscall error with the original object unchanged. Its safe `Deref`/`DerefMut` implementations at lines 61–76 assume the mapping remains valid until drop; its destructor at lines 79–83 unmaps the old address.
...
**Contract.** [KERNEL-SPEC.md](../../KERNEL-SPEC.md), lines 199–203 (R4), permits a reply to arrive with some handles missing: successful slots and words are delivered, missing slots are zero, and `call` returns `OutOfMemory`.

**Implementation.** [kernel/src/message.rs](https://github.com/sirmick/redoubt/blob/main/kernel/src/message.rs), lines 1219–1227, installs the handles that fit and writes the reply with that status. [libs/rt/src/ipc.rs](https://github.com/sirmick/redoubt/blob/main/libs/rt/src/ipc.rs), line 130, uses `nothing(syscall(&call))?`, returning before the reply record is decoded.
...
**Contract.** [KERNEL-SPEC.md](../../KERNEL-SPEC.md), lines 341–360, requires output-record writability to be checked during decoding, before handle, permission, resource, or delivery effects.

**Implementation.** The shared call/send path in [kernel/src/message.rs](https://github.com/sirmick/redoubt/blob/main/kernel/src/message.rs), line 625, only uses `read_record`. In [kernel/src/redoubt.rs](https://github.com/sirmick/redoubt/blob/main/kernel/src/redoubt.rs), lines 192–195, that validates with `write = false`. Calls later write their reply into the same record: `message.rs`, lines 1223–1226. Writability is checked by `write_record` only at that point (`redoubt.rs`, lines 204–208).
...
**Evidence.** [tests/unsafe-budget.toml](https://github.com/sirmick/redoubt/blob/main/tests/unsafe-budget.toml), lines 11, 25, 70, 76, and 82, names:
...
[tools/testbench/src/budget.rs](https://github.com/sirmick/redoubt/blob/main/tools/testbench/src/budget.rs), lines 46–54, returns success for a missing extensionless path because it is neither a directory nor an `.rs` file.
...
The host test double is also candid about its limitations: [libs/rt/tests/common/mod.rs](https://github.com/sirmick/redoubt/blob/main/libs/rt/tests/common/mod.rs), lines 3–11, omits budgets, charging, label enforcement, fair waiting, and lender unmapping. Passing those tests cannot validate the missing behaviors. C1 and C2 show why tests at the real kernel/runtime boundary are necessary alongside host tests.
...
**Evidence.** [INIT.md](../../INIT.md), lines 76–108, makes `confined` a boot-wide prohibition on differing label sets sharing servers, endpoints, devices, or cores. It explicitly treats the unlabelled steward as a domain of its own. Yet [CONTAINMENT.md](../../CONTAINMENT.md), lines 58–67 and 78–89, requires the steward to communicate with labelled reader/writer helpers for declassification and push. [CAPABILITIES.md](../../CAPABILITIES.md), lines 177–185, also requires labelled requests and an owner approval path.
...
**Evidence.** [TENETS.md](../../TENETS.md), lines 24–26, says an individual agent's authority cannot exceed its initial grants plus human-approved additions. [CAPABILITIES.md](../../CAPABILITIES.md), lines 19–25, permits copying every handle and narrowing delegation. Meanwhile, `TENETS.md`, lines 30–32, and [ANSWERS.md](../../ANSWERS.md), lines 233–240, deliberately treat equal-label budgets as one trust domain.
...
**Consequence for implementation.** Intended, correct delegation can violate the literal per-agent guarantee and the authority-expansion verdict in [GAME.md](../../GAME.md), lines 27–29. The specification and its proposed evaluation would disagree about permitted behavior.
...
**Evidence.** [KERNEL-SPEC.md](../../KERNEL-SPEC.md), lines 253–259, selects the lowest pass and wakes a budget at `max(own pass, current minimum)`. [RESOURCES.md](../../RESOURCES.md), lines 40–51, claims drivers and the steward wait at most about one `SLICE`.
...
**Evidence.** [kernel/src/message.rs](https://github.com/sirmick/redoubt/blob/main/kernel/src/message.rs), lines 1208–1217, discards an abandoned reply and returns `Ok(())`. [libs/rt/src/server/typed.rs](https://github.com/sirmick/redoubt/blob/main/libs/rt/src/server/typed.rs), lines 185–194, forwards that reply result. But [servers/keyd/src/server.rs](https://github.com/sirmick/redoubt/blob/main/servers/keyd/src/server.rs), lines 125–135, and [libs/rt/src/server/ninep.rs](https://github.com/sirmick/redoubt/blob/main/libs/rt/src/server/ninep.rs), lines 495–505, undo a newly minted record only if replying returns an error.
...
**Test gap.** [servers/keyd/src/server_tests.rs](https://github.com/sirmick/redoubt/blob/main/servers/keyd/src/server_tests.rs), lines 724–735, tests rollback by calling `forget_badge` directly. It proves cleanup works when called, but not that actual abandoned IPC invokes it.
...
**Evidence.** [servers/consoled/src/bin/consoled.rs](https://github.com/sirmick/redoubt/blob/main/servers/consoled/src/bin/consoled.rs), lines 120–123, supplies an unsupported-operation callback that calls `request.reply(MALFORMED, &[])` directly. [libs/rt/src/server/ninep.rs](https://github.com/sirmick/redoubt/blob/main/libs/rt/src/server/ninep.rs), lines 488–490, dispatches unknown opcodes to that callback, bypassing the normal `finish` path that closes unused request handles. [libs/rt/src/ipc.rs](https://github.com/sirmick/redoubt/blob/main/libs/rt/src/ipc.rs), lines 267–275, sends the reply but does not close received handles automatically. The handles are plain entries, with no request destructor that reclaims them.
...
**Evidence.** [kernel/src/arch/riscv/irq.rs](https://github.com/sirmick/redoubt/blob/main/kernel/src/arch/riscv/irq.rs), lines 220 and 234, dispatches both new and legacy calls. [kernel/src/redoubt.rs](https://github.com/sirmick/redoubt/blob/main/kernel/src/redoubt.rs) explicitly describes coexistence. The legacy syscall/server implementation and ABI syscall file together contain more than 4,000 lines; this is a scale indicator, not a promised deletion count.
...
**Evidence.** [Cargo.toml](https://github.com/sirmick/redoubt/blob/main/Cargo.toml), lines 18–19, includes `libs/flatipc` and `libs/flatipc-derive`. The reviewer checked `cargo metadata --no-deps` and searched source/manifests: neither has another in-repository consumer, except `flatipc` using its derive crate. Together they contain roughly 1,232 Rust lines including tests.
...
**Evidence.** [BUILD-PLAN.md](../../BUILD-PLAN.md), line 594, says R4/D1 are building, while lines 597 and 599 call them merged. [ARCHITECT-NOTES.md](../../ARCHITECT-NOTES.md), line 24, calls question 160 open, while [ANSWERS.md](../../ANSWERS.md), line 414, records the owner's decision. [STATUS.md](../../STATUS.md), line 18, says no virtio driver is built, although [servers/blkd/src/lib.rs](https://github.com/sirmick/redoubt/blob/main/servers/blkd/src/lib.rs) contains a substantial implementation with integration explicitly pending in its binary.
...
**Evidence.** [PLAN.md](../../PLAN.md), lines 10–28, describes a thin slice that nevertheless includes beamlet, IEx, init, storage, networking, steward/key service, SSH, and the leased agent. Final acceptance in [BUILD-PLAN.md](../../BUILD-PLAN.md), lines 583–587, depends on the whole stack.
...
**Evidence.** [OS-API.md](../../OS-API.md), lines 37–54, proposes a nine-module `redoubt-os` facade. Existing client/runtime mechanisms live in [libs/rt/src/client.rs](https://github.com/sirmick/redoubt/blob/main/libs/rt/src/client.rs) and [libs/rt/src/handle.rs](https://github.com/sirmick/redoubt/blob/main/libs/rt/src/handle.rs). The draft API's rejection of a `std::fs` shim also needs reconciliation with [PLAN.md](../../PLAN.md), lines 131–137, which still describes a future Rust `std` backend.
...
following [the architect Q&A protocol](https://github.com/sirmick/redoubt/blob/main/.pi/skills/architect-qa/SKILL.md). The round opened
[questions 164–168](QUESTIONS.md#from-the-astra-architect-round-2026-09-22), each with a
...
The round is recorded in [SWARM.md](../../SWARM.md#cross-cutting-review-records) and
[HISTORY.md](../../HISTORY.md). It does not clear existing package review debt or certify any
...
[WP-IPC1](../../BUILD-PLAN.md) scopes ABI/kernel/model/runtime/server implementation and its
...
## 11. Remote branch recovery review — 2026-09-22

The owner supplied four remote branches and explicitly prohibited merging them. They were
fetched and inspected against primary `fe807fc4b`; no branch was merged, checked out, or
copied into the implementation. This is a recovery inventory, not package acceptance.

| Source tip | Current destination | Recovery assessment |
| --- | --- | --- |
| `origin/wp-d1` — `e4d22b980` | `redoubt/blkd/` → `servers/blkd/` | Already recovered by `8681f2648`; preserve newer fixes. |
| `origin/wp-r4` — `33a46e010` | `redoubt/{bootfsd,consoled}/` → `servers/{bootfsd,consoled}/`; shared `redoubt/rt/` → `libs/rt/` | Already recovered through R1c/R4b, including `69466924c`; preserve newer IPC and server fixes. |
| `origin/wp-k4` — `6ddf06786` | Kernel paths remain `kernel/`; `redoubt/test-programs/` → `tests/programs/`; `redoubt/tests/` → `tests/` | Substantial missing process-lifecycle implementation and kernel tests; selective port needed. |
| `origin/wp-m0` — `674831cf9` | `redoubt/model/` → `model/`, with workspace registration | Missing executable model recovered as source, but not current-spec conformant or integrated. |

Other moved paths are `planning/redoubt/` → `docs/` and `redoubt/wire/` → `libs/wire/`.
Old kernel references to `xous_kernel` must follow the current `redoubt_abi` naming. Do not
replace current manifests, generated codecs or unsafe-budget configuration with old versions.
No `wp-d3` branch was among the four; its existing external claim remains unverified.

### Reuse and blockers

1. **Do not recover D1/R4 twice.** Every file in the three old server trees has a current
   counterpart. Blob comparisons found 27 identical / 7 changed files for blkd, 2 / 5 for
   bootfsd, and 3 / 4 for consoled; none absent. Differences include path fixes, subsequent
   simplifications, server hardening and IPC outcome migration. The recovered commits are
   already ancestors of `origin/main`. **The bench registration is not fully recovered:**
   `blkd-build.toml`, `blkd-host-tests.toml`, `bootfsd-build.toml`, `consoled-build.toml` and
   `r4-host-tests.toml` exist under the branches' `redoubt/tests/` but not current `tests/`.
   Current `tests/host-tests.toml` runs only signing and testbench. Reuse those definitions
   with current comments/paths so server host tests and both-width builds run automatically;
   section 9's manual server checks do not substitute for permanent bench registration.
   Their server unsafe-budget entries were also omitted: the old D1 limit is 4 for blkd,
   and R4's combined bootfsd/consoled limit is 0, all with zero undocumented uses. Restore
   coverage at `servers/` paths and verify it; do not copy R4's old runtime ceiling of 11
   over primary's reviewed 9. A fail-closed checker can reject a bad configured root, but
   cannot detect an entirely omitted component without a coverage inventory.
2. **K4 supplies the process work we need.** Its two package commits, `591f578a9` and
   `6ddf06786` (base `cc34cc756`), add `kernel/src/process.rs`, process/thread syscalls,
   creator-paid process objects and exit notices, blame snapshots and lifecycle tests.
   The test commit also fixes permission-fault retry loops and stale address-space state
   when reusing a PID. These are useful implementation candidates, not freshly reproduced
   test results. Current primary still refuses the new process/thread syscalls.
3. **K4's IPC baseline is obsolete.** Its `kernel/src/arch/riscv/mem.rs`, `message.rs` and
   `redoubt.rs` predate answers 167–168 and the same-PID borrowed-alias fix. Whole-file
   replacement would lose protected borrower mappings, identity-checked return, observable
   ownership/delivery and completion rollback. Port the process hooks into current code;
   preserve primary's mapping and IPC semantics and test their composition with process exit.
4. **K4 is not acceptance-complete.** `redoubt/tests/process.toml` covers causes, blame,
   startup and thread limits, but does not establish the full planned hostile process-map,
   handle-list, weight-zero, badged-exit-endpoint and notice-exhaustion gates. The branch
   changes no loader or testbench implementation, so it does not deliver the required clean
   bundle-file readback. The exit-notice-based `wx` verdict and remaining R8/R9/I8 cases
   also need checking/completion against BUILD-PLAN, not assumptions from the branch name.
5. **The model needs semantic reconciliation, not just relocation.** Its `src/sched.rs`
   still implements two `first` tiers, explicitly removed by accepted answer 103. Its
   `src/syscall.rs` represents completion as `Result<Ret, Error>` and expressly omits ABI
   record buffers; `src/kernel.rs:2835` replies with unit and does not expose committed
   partial replies, lend disposition or delivery masks. Update the kernel model, ghost
   checks, mutations and trace format to answers 167–168, including output validity and
   rollback. The model does contain process creation/exit and exit notices; it is not a
   missing lifecycle model, but an older one. Historical mutation-test claims are not
   evidence that it conforms to today's contracts.

### Three-angle reconnaissance

Three independent Sol readers completed the editor/consistency, defensive and simplifier
reviews. They agree on selective K4 recovery and retaining the newer D1/R4 implementation.
The editor identified the model's obsolete outcome/trace contract and the missing K4
acceptance cases. The defensive reader identified the borrowed-mapping regression risk and
required native tests for server exit with a live lend, caller exit/abandonment, thread exit
with surviving siblings, same-process loans and PID reuse. The simplifier verified that
D1/R4 source differences preserve later deletions and hardening; follow-up inspection
confirmed the missing five bench definitions and server unsafe-budget registrations. These are reconnaissance conclusions, not
three-review approval of an implementation port.

Recommended next work: restore the missing server bench and unsafe-budget registrations as a small tooling
package; selectively port K4 onto current primary in an isolated worktree, with one kernel
writer; recover and reconcile the host model independently. Retain the
current IPC regressions and add process-exit/lend cleanup cases before acceptance. Each
candidate still needs the swarm's three reviews, focused tests, full bench, unchanged unsafe
budgets and rv32 compilation. The model also needs its property/mutation acceptance run.
This recommendation does not authorize a merge or expand milestone 1 to full SMP; PLAN.md
places SMP after milestone 1, and the earlier concurrency-gate wording needs reconciliation.

Validation this pass: source/history inspection and mapped blob comparisons only; no new
builds, boot tests or model property tests. The prior 99-execution bench result belongs to
primary `fe807fc4b`, not these unported branches.
