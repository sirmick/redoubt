...
- **Crates are judged, not assumed.** A virtio crate is read and judged under tenet 5 like any
  other dependency; `virtio-drivers` was read at 0.13.0 and rejected for the drivers here, which are
  ours — the reasoning is in `blkd`'s section below, and `netd` (WP-D3) inherits it rather than
  re-arguing it. A driver is audited as TCB while it runs.
...
again, and more than five restarts within sixty seconds reboot the machine (INIT.md). So a corrupt
or unpartitioned disk is a reboot loop, not a degraded boot. That is deliberate — a volume `blkd`
