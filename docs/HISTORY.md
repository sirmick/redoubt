...
- **Recording the use-case direction as answers 150-153** (2026-09-22, architect protocol): the
  direction above was written straight into the notes with no numbered question, so four unbuilt
  packages (WP-R3, WP-D2, WP-D3, WP-S2) had decisions to build on and nothing in the Q&A record to
  cite (SWARM.md rule 7). Questions 150-153 record it without re-deciding anything: **150** names
  and defines the isolation-unit property; **151** makes TENETS.md (The adversary) the single
  statement of the covert-channel non-claim; **152** pins what `confined` makes `init` compare; and
  **153** defines the steward push as the mirror of declassification. Applied to TENETS.md,
  CONTAINMENT.md, CAPABILITIES.md and INIT.md; no mechanism changed.
