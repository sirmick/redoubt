...
| Memory and devices | Typed W^X page tables, zeroed anonymous RAM, default-deny legacy device grants; new MMIO/DMA/reset handles; the kernel resets every DMA device a dying process could reach before its frames are reused, and quarantines them for good if a device does not confirm (WP-K5b). | User code retains a writable kernel-only physmap alias. DMA drivers remain trusted while they live; non-virtio DMA devices are never reset, so their drivers get no restart. Device-policy questions 142–146 are open. |
...
| Scheduling and processes | Native process/thread creation, startup and exit; creator-paid notices, blame, loan cleanup and PID reuse; kernel-owned timer, one preemptive stride queue over every budget, timeouts and budget deadlines with measured latency targets (WP-K5); a two-hart lock spike. | No full SMP scheduler. Legacy interface removal (WP-K6). |
| Native libraries and servers | sys/rt/wire/signing, pure-Rust littlefs, keyd, bootfsd, consoled and in-tree virtio-blk/blkd have host tests. netd (virtio-net) and ipd:lan (TCP over /net, scoped) boot on the bench through a test rig (WP-D3; bench-only until R3's manifest boot). | Init/startup integration, fsd, network/steward/SSH stack and Redoubt beamlet platform. Console typed size/resize are target behavior; typed parking awaits 163. |
