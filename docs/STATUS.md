...
| 2 `unsafe` budget | (enforced) Every `unsafe` in the kernel, loader and `paging` has a SAFETY justification (0 undocumented; 211 undocumented at the fork). Budgets: `paging` 12, loader 17, SBI/PLIC/timer backends 15, RISC-V arch 10, kernel core 20. Totals only fall. |
...
| `redoubt-ipc` | The IPC rules across address spaces: badges and stamps, receive rights, fair waiting, open calls, abandoned notices, a full handle table refused at delivery |
| `redoubt-revoke` | Revocation reaches queued messages, taken calls and handles carried in messages (R10, I3) |
| `redoubt-dead` | A server's death: taken calls get `Dead`, queued senders wait for the restart (R4b) |
| `redoubt-tight` | A receiver carved to its exact minimum is `Refused` and spends nothing doing it (R4) (attack) |
| `redoubt-ipc-attack` | Hostile IPC: stolen receive rights, forged badges and message ids, foreign mints (attack) |
