...
compiler, inside a microkernel OS (redoubt64) and, for development and testing, on POSIX.
...
- Toolchains are prebuilt archives from builds.hex.pm, checked by SHA-256, unpacked under
  the repo root's `toolchains/`. `tools/env.sh` puts them on `PATH`.
...
- **CPU**: within a VM, reductions preempt every process (including call-free loops). Between
  VMs, CPU share is the host scheduler's job (on Redoubt, the kernel's per-process scheduling); the
...
- The limits are measurements, not an allocator: a single native that allocates a lot at once
  ...
  embedder's allocator; on Redoubt, the process's memory quota from the kernel.
...
- The `Files` trait is synchronous and file-shaped, a first step towards the design below: its
  ...
  so a Redoubt platform implements it with a 9P client, and it can later fold into the one
...
capabilities (redoubt `docs/NAMESPACES.md`). beamlet follows that:
...
- **TCP** (`vm/lib/gen_tcp.erl`, `vm/lib/beamlet_tcp.erl`): `gen_tcp` is replaced by a front for one
  ...
  the platform provides one (on Redoubt: a backend talking to the network server). A VM does not
