...
{
    let server_address = redoubt_address();
    let pid = startup.pid;
    ...
    let thread_main = std::thread::Builder::new()
        .name(args.name)
        .spawn(move || {
            set_redoubt_address(server_address);
            THREAD_ID.with(|tid| *tid.borrow_mut() = 1);
            PROCESS_ID.with(|p| *p.borrow_mut() = pid);
            REDOUBT_SERVER_CONNECTION.with(|xsc| {
                let mut xsc = xsc.borrow_mut();
                match redoubt_connect_impl(server_address, &init.key) {
                    Ok(a) => {
                        *xsc = Some(a);
                        Ok(())
                    }
...
pub fn create_process_post(
    ...
    let server_env = format!("{}", redoubt_address());
    ...
    Command::new(shell)
        .args(&args)
        .env("REDOUBT_SERVER", server_env)
        .env("REDOUBT_PID", pid_env)
        .env("REDOUBT_PROCESS_NAME", process_name_env)
        .env("REDOUBT_PROCESS_KEY", process_key_env)
        .spawn()
        .map(ProcessHandle)
        .map_err(|_| {
...
thread_local!(static REDOUBT_SERVER_CONNECTION: RefCell<Option<ServerConnection>> = RefCell::new(None));
...
fn default_redoubt_address() -> SocketAddr {
    std::env::var("REDOUBT_SERVER")
        .map(|s| {
            s.to_socket_addrs()
...
fn default_process_key() -> ProcessKey {
    std::env::var("REDOUBT_PROCESS_KEY").map(|s| s.as_str().into()).unwrap_or(ProcessKey([0u8; 16]))
}
...
pub fn set_redoubt_address(new_address: SocketAddr) {
    NETWORK_CONNECT_ADDRESS.with(|nca| {
        let mut address = nca.borrow_mut();
        *address = Some(new_address);
        REDOUBT_SERVER_CONNECTION.with(|xsc| *xsc.borrow_mut() = None);
    });
}

/// Get the network address for this particular thread.
fn redoubt_address() -> SocketAddr {
    NETWORK_CONNECT_ADDRESS.with(|nca| *nca.borrow()).unwrap_or_else(default_redoubt_address)
}
...
{
    let server_address = redoubt_address();
    let server_connection = REDOUBT_SERVER_CONNECTION.with(|xsc| xsc.borrow().as_ref().unwrap().clone());
    ...
    Ok(std::thread::Builder::new()
        .spawn(move || {
            ...
            REDOUBT_SERVER_CONNECTION.with(|xsc| *xsc.borrow_mut() = Some(server_connection));
...
pub fn ensure_connection() -> core::result::Result<(), crate::Error> {
    REDOUBT_SERVER_CONNECTION.with(|xsc| {
        ...
        if xsc.is_none() {
            NETWORK_CONNECT_ADDRESS.with(|nca| {
                let addr = nca.borrow().unwrap_or_else(default_redoubt_address);
                let pid1_key = PROCESS_KEY.with(|pk| *pk.borrow()).unwrap_or_else(default_process_key);
                match redoubt_connect_impl(addr, &pid1_key) {
                    Ok(a) => {
                        *xsc = Some(a);
                        Ok(())
                    }
...
fn redoubt_connect_impl(addr: SocketAddr, key: &ProcessKey) -> core::result::Result<ServerConnection, ()> {
    // eprintln!("Opening connection to Redoubt server @ {} with key {:?}...", addr, key);
    ...
    match TcpStream::connect(addr) {
        ...
        Err(_e) => {
            // eprintln!("Unable to connect to Redoubt server: {}", _e);
            // eprintln!(
            //     "Ensure Redoubt is running, or specify this process as an argument to the kernel"
            // );
            Err(())
        }
    }
    ...
    REDOUBT_SERVER_CONNECTION.with(|xsc| {
        THREAD_ID.with(|tid| {
            ...
            let xsc_as_mut = xsc_borrowed.as_mut().expect(
                "not connected to server (did you forget to create a thread with redoubt_abi::create_thread()?)",
            );
            loop {
                _redoubt_syscall_to(nr, a1, a2, a3, a4, a5, a6, a7, &call, xsc_as_mut);
                _redoubt_syscall_result(&mut ret, *tid.borrow(), xsc_as_mut);
                match ret {
...
fn _redoubt_syscall_result(ret: &mut Result, thread_id: TID, server_connection: &ServerConnection) {
