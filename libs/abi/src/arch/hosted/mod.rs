...
lazy_static::lazy_static! {
    ...
    pub(crate) static ref PROCESS_KEY: ProcessKey = {
        ...
        let seed = match std::env::var("REDOUBT_SEED") {
            Ok(s) => {
        ...
        std::env::var("REDOUBT_PROCESS_KEY")
        .map(|s| s.as_str().into())
    ...
    static ref SERVER_CONNECTION: ServerConnection = {
        ...
        let mut conn = TcpStream::connect(*NETWORK_CONNECT_ADDRESS).expect("unable to connect to Redoubt kernel");
    ...
    static ref PROCESS_ID: PID = {
        use std::str::FromStr;
        PID::from_str(&std::env::var("REDOUBT_PID")
            .expect("missing environment variable REDOUBT_PID"))
            .expect("REDOUBT_PID environment variable was not valid")
    };
...
pub fn set_redoubt_address(new_address: SocketAddr) { *CHILD_PROCESS_ADDRESS.lock().unwrap() = new_address; }
