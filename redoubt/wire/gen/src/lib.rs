...
//! and `redoubt/wire/tables/*.md` (skipping fenced code blocks), finds each message table
//! marked `<!-- wire: NAME -->` and each error table marked `<!-- wire-errors: NAME -->`, and
//! emits `redoubt/wire/src/proto/NAME.rs`, `redoubt/wire/elixir/proto/NAME.ex`, and
//! `redoubt/wire/src/proto/mod.rs` listing the Rust modules. The generated files are checked
...
//! `redoubt/wire/tables/example.md`.
//!
...
mod tests {
    ...
    fn fenced_r1b_tables_parse() {
        let mut found = Vec::new();
        for (note, name) in [("planning/redoubt/NAMESPACES.md", "ninep_common"), ("planning/redoubt/INIT.md", "startup")] {
