...
impl TypedServer<Example> for Store {
    fn handle<'s>(
        ...
        request: Message<'_>,
        handles: &[Handle],
    ) -> Result<(Reply<'s>, Handles), ErrorCode> {
        let none = Handles::new();
        match request {
            Message::Small(Small { a, b }) => {
                Ok((Reply::Small(SmallReply { c: u32::from(a) + u32::from(b) }), none))
            }
            Message::Named(Named { name: "deny", .. }) => Err(ErrorCode::Denied),
            Message::Named(Named { id, .. }) => {
                Ok((Reply::Named(NamedReply { id: id ^ caller.account as u32 }), none))
            }
            Message::Blob(Blob { data, .. }) => {
                self.stored = data.to_vec();
                Ok((Reply::Blob(BlobReply {}), none))
            }
            Message::Read(Read { offset, count }) => {
                ...
                Ok((Reply::Read(ReadReply { data: &self.stored[start..end] }), none))
            }
            Message::Grant(Grant { .. }) => {
                self.kept.push(handles[0]);
                Ok((Reply::Grant(GrantReply {}), Handles::from_slice(&handles[1..]).unwrap()))
            }
            _ => Err(ErrorCode::NotFound),
        }
    }

    fn malformed(&self) -> ErrorCode { ErrorCode::LastError }
}
...
/// afterwards, the reply's words and handles, and the handles returned unread.
fn round_trip(
    store: &mut Store,
    request: Message<'_>,
    handles: &[Handle],
    ...
    let ((words, reply_handles), unread) =
        answer::<Example, Store>(store, &caller(), &words, &handles, &mut buf);
    (buf, words, reply_handles, unread)
}
...
fn handles_travel_and_unread_ones_come_back() {
    ...
    assert_eq!((reply_handles.as_slice(), unread.as_slice()), (&[h(6)][..], &[][..]));
    ...
    assert_eq!(words, ErrorCode::LastError.encode());
    assert!(reply_handles.as_slice().is_empty());
...
fn malformed_requests_and_oversized_replies() {
    let mut store = Store::default();
    let bad = ErrorCode::LastError.encode();
    let mut buf = [0u8; 16];
    let none = Handles::new();
    for words in [[0, 0, 0, 0], [99, 0, 0, 0], [3, 1 << 32, 0, 0], [5, 100, 0, 0], [3, 0, 0, 1]] {
        let ((reply, _), _) = answer::<Example, Store>(&mut store, &caller(), &words, &none, &mut buf);
        assert_eq!(reply, bad, "{words:?}");
    }
    // A buffer-shaped request with no buffer.
    let ((reply, _), _) = answer::<Example, Store>(&mut store, &caller(), &[5, 6, 0, 0], &none, &mut []);
    assert_eq!(reply, bad);
