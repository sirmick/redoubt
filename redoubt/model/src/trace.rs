...
pub const HEADER: &str = "redoubt-model-trace 1";
...
impl Names {
    ...
    fn result(&mut self, pid: u64, call: Option<&Syscall>, r: &Result<Ret, Error>, k: &Kernel) -> String {
        ...
        match r {
            ...
            Ret::Reply { words, handles } => format!(
                "ok reply words={} handles={}",
                Names::list(words.iter().map(|w| format!("{w}"))),
                Names::list(handles.iter().map(|x| h(*x)))
            ),
            Ret::Message(m) => {
