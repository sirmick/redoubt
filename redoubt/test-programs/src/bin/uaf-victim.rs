...
pub extern "C" fn _start() -> ! {
    ...
    let page = xous::map_memory(None, None, 4096, MemoryFlags::R | MemoryFlags::W).expect("map failed");
    unsafe {
        core::ptr::copy_nonoverlapping(VICTIM_SENTINEL.as_ptr(), page.as_mut_ptr(), VICTIM_SENTINEL.len());
    }

    let cid = xous::connect(SID::from_bytes(HOLDER_ADDRESS).unwrap()).expect("connect failed");
    xous::create_thread_1(terminator, 0).expect("couldn't spawn terminator");
    log!(logger, "[victim] PID {} lending page, then dying", xous::current_pid().unwrap());
    ...
    xous::send_message(cid, Message::new_lend_mut(HOLD, page, None, MemorySize::new(4096)))
        .expect("lend failed");
