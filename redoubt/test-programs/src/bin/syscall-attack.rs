...
//! clean error. See `redoubt/tests/syscall-attack.toml`; the case's verdict is survival only,
...
use xous::{MemoryFlags, Message, SysCall};
...
pub extern "C" fn _start() -> ! {
    ...
    let grow = xous::rsyscall(SysCall::IncreaseHeap(2 * 4096, rw | MemoryFlags::X));
    let shrink = xous::rsyscall(SysCall::DecreaseHeap(4096));
    ...
    let big = xous::map_memory(None, None, 5 << 20, rw).expect("couldn't reserve 5 MiB");
    let lend = xous::send_message(logger.cid, Message::new_lend(op::PRINT, big, None, None));
    xous::unmap_memory(big).expect("couldn't unmap the 5 MiB range");
