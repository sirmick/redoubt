...
fn child(arg: usize) -> ! {
    ...
    match role {
        ...
        R_OTHER_THREAD => {
            ...
            rd::thread_create(entry_of(fault_thread), stack + 4 * rd::PAGE_SIZE - 16, 0)
...
fn entry_of(f: extern "C" fn(usize) -> !) -> usize {
    f as *const () as usize
}
...
fn threads(out: &mut Out) {
    ...
    // A stack pointer of 0 is no stack at all.
    check!(
        out,
        rd::thread_create(entry_of(exiting_thread), 0, 0) == Err(Error::InvalidArgument),
        "thread_create refuses a stack pointer it cannot use"
    );
...
extern "C" fn parking_thread(_arg: usize) -> ! {
    test_programs::park()
}
