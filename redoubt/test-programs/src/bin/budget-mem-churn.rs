...
fn map_touch_unmap() {
    let range = xous::map_memory(None, None, PAGES * 4096, MemoryFlags::R | MemoryFlags::W).expect("map");
    for page in 0..PAGES {
        // SAFETY: this program's own fresh mapping.
        unsafe { (range.as_mut_ptr().add(page * 4096) as *mut u64).write_volatile(page as u64 + 1) };
    }
    xous::unmap_memory(range).expect("unmap");
}

fn lend(logger: &Logger, page: &mut Page) {
    page.clear();
    page.write_str("churn").ok();
    xous::send_message(logger.cid, Message::new_lend_mut(op::UPPERCASE, page.range, None, page.valid()))
        .expect("lend");
}
