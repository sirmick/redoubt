...
pub extern "C" fn _start() -> ! {
    ...
    let sid = redoubt_abi::create_server_with_address(victim::ADDRESS).expect("couldn't create the victim's server");
    loop {
    ...
    let range = redoubt_abi::map_memory(None, None, len, MemoryFlags::R | MemoryFlags::W).expect("map_memory");
