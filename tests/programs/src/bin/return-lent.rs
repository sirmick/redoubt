...
use redoubt_abi::{MemoryAddress, MemoryFlags, MemoryRange, MemorySize, Message};
...
fn clobber(_arg: usize) -> ! {
    ...
    let unmapped = redoubt_abi::unmap_memory(range);
    let remapped =
        redoubt_abi::map_memory(None, MemoryAddress::new(LENT_ADDR), 4096, MemoryFlags::R | MemoryFlags::W)
            .map(|_| ());
...
pub extern "C" fn _start() -> ! {
    ...
    let page = redoubt_abi::map_memory(None, MemoryAddress::new(LENT_ADDR), 4096, MemoryFlags::R | MemoryFlags::W)
        .expect("map");
    ...
    let sid = redoubt_abi::SID::from_bytes(return_lent::ADDRESS).unwrap();
    let borrower = redoubt_abi::connect(sid).expect("connect to borrower");
    redoubt_abi::create_thread_1(clobber, 0).expect("spawn clobber thread");

    // Mutable lend: blocks until the borrower returns it.
    let result =
        redoubt_abi::send_message(borrower, Message::new_lend_mut(1, page, None, MemorySize::new(text.len())));
