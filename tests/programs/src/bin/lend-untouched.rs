...
use redoubt_abi::{MemoryMessage, MemoryRange, MemorySize, Message};

fn untouched(pages: usize) -> MemoryRange {
    redoubt_abi::map_memory(None, None, pages * 4096, redoubt_abi::MemoryFlags::R | redoubt_abi::MemoryFlags::W)
...
pub extern "C" fn _start() -> ! {
    ...
    redoubt_abi::send_message(server, Message::new_lend(op::PRINT, page, None, None)).expect("lend");
    ...
    redoubt_abi::unmap_memory(unsafe { MemoryRange::new(second, 4096) }.expect("range")).expect("unmap the hole");
    let refused = redoubt_abi::send_message(server, Message::new_lend(op::PRINT, straddle, None, None)).is_err();
    ...
    redoubt_abi::send_message(server, Message::new_lend(op::PRINT, first, None, MemorySize::new(text.len())))
        .expect("re-lend");
