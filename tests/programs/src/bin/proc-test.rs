...
fn blame(out: &mut Out, parent: &Parent, budget_a: u32, budget_b: u32) {
    ...
    // yet run its own `process_exit` still holds its budget's process slot, and the next
    // scenario reuses `budget_a`/`budget_b` right away. Without this wait, reuse races the
    ...
    let scenario = |role: u8, value: u8, callers: &[(u32, u8, u8)]| -> Option<ExitNotice> {
        ...
        // No scenario here uses more than two callers.
