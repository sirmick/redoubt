//! Drives every Redoubt message type against `log-server`, and so every page-table
...
pub extern "C" fn _start() -> ! {
    ...
    redoubt_abi::send_message(cid, Message::new_scalar(op::PRINT_SCALARS, 1, 2, 3, 4)).expect("scalar");

    // BlockingScalar: the reply comes back in registers. Use values that need 64 bits
    // where they are available.
    let big = 1usize << (usize::BITS - 8);
    let sum = match redoubt_abi::send_message(cid, Message::new_blocking_scalar(op::SUM, big, big, 3, 4)) {
        Ok(redoubt_abi::Result::Scalar1(sum)) => sum,
        other => {
    ...
    redoubt_abi::send_message(cid, Message::new_lend_mut(op::UPPERCASE, scratch.range, None, scratch.valid()))
