...
//! says whether any held data (docs/testbench.md, "Rule F (trusted verdicts)"); this program's
//! own reports are progress only. There is no call that maps RAM by physical address to try.
//!
...
//! physical addresses (the model's invariant I9; kernel/memory.md, "Backing and zeroing").
    ...
    for _ in 0..PAGES {
        match rd::map_anon(rd::PAGE_SIZE, rd::rw()) {
            Ok(page) => {
                ...
                // one it has never touched must be served, not a kernel panic
