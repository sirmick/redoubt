...
use redoubt_abi::{MemoryAddress, MemoryFlags, MemoryRange, SysCall};

// Devices on QEMU virt this program was NOT granted.
const UART: usize = 0x1000_0000;
const POWEROFF: usize = 0x0010_0000;

fn try_map(base: usize) -> Result<redoubt_abi::Result, redoubt_abi::Error> {
...
fn mapped(result: &Result<redoubt_abi::Result, redoubt_abi::Error>) -> Option<MemoryRange> {
    match result {
        Ok(redoubt_abi::Result::MemoryRange(range)) => Some(*range),
        _ => None,
    }
}
...
pub extern "C" fn _start() -> ! {
    ...
    let ram = redoubt_abi::map_memory(None, None, 4096, MemoryFlags::R | MemoryFlags::W);
    ...
    for irq in [0usize, 10, 6] {
        let result = redoubt_abi::claim_interrupt(irq, keep_it, core::ptr::null_mut());
